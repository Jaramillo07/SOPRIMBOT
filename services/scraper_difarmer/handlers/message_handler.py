...
class MessageHandler:
    ...
    def es_mensaje_a_ignorar(self, mensaje: str) -> bool:
        ...
        patrones_no_relevantes = [
            ...
            # Nuevos patrones para expresiones coloquiales y ocio
            r"(?:vamos|vámonos|anda|andamos|salimos)[\s,]*(?:a|por|x|al)[\s,]*(?:cheve|cheves|cerveza|cervezas|bebidas|tragos|drink|drinks|cine|movie|película|mall|centro)",
            r"(?:cervecear|pistear|beber|tomar|echar|peda|peda\w+|fiesta|party)",
            r"(?:salir|rolar|rodar|dar la vuelta|dar un rol|pasear)",
            r"(?:antro|bar|cantina|botanero)",
            r"\b(?:chela|chelita|cheve|chevecita|cerveza|birra)\b",
            r"(?:a qué hora|a q h[o]?ra).*(?:sales|vienes|llegas|estás|estas|vas)"
        ]
        ...
        # También comprobamos expresiones específicas de ocio
        expresiones_ocio = ["vamos x cheves", "vamos por cheves", "vamos a tomar", "vamos al cine"]
        for expr in expresiones_ocio:
            if expr in m:
                logger.info(f"Ignorado por expresión de ocio: {expr}")
                return True
    ...
    async def procesar_mensaje(self, mensaje: str, phone_number: str) -> dict:
        ...
        # 1) Primero verificar con la detección local básica
        if self.es_mensaje_a_ignorar(mensaje):
            logger.info(f"Mensaje ignorado localmente: '{mensaje}'")
            # No enviamos respuesta para mensajes personales o coloquiales
            return {
                "success": True,
        ...
        # 2) Si no es ignorado localmente, intentar con Gemini para una detección más avanzada
        try:
            ignorar_segun_gemini = self.gemini_service.detectar_mensaje_ignorable(mensaje)
            if ignorar_segun_gemini:
                logger.info(f"Mensaje identificado como charla casual/coloquial por Gemini: '{mensaje}'")
                # No enviamos respuesta para mensajes personales o coloquiales
                return {
                    "success": True,
                    "message_type": "ignorado_gemini",
                    "respuesta": None
                }
        except Exception as e:
            logger.error(f"Error al detectar si el mensaje es ignorable con Gemini: {e}")
            # Continuamos con el procesamiento normal en caso de error
        
        # 3) Detectar tipo de mensaje localmente primero
        ...
        # 4) Si es consulta de producto, hacemos scraping
        ...
        # 5) En cualquier otro caso, respuesta general
